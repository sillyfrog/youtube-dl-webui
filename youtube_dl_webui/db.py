...
class DataBase(object):
    ...
    def new_task(self, url, ydl_opts):
        ...
        self.db.execute(
            "INSERT INTO task_info (tid, url, create_time) VALUES (?, ?, ?)",
            (tid, url, time()),
        )
        self.db.execute(
            "INSERT INTO task_ydl_opt (tid, url, opt) VALUES (?, ?, ?)",
    ...
    def finish_task(self, tid, elapsed, finish_time=time()):
        ...
        db_data = {
            "task_info": {"state": state, "finish_time": finish_time,},
            "task_status": {
                "pause_time": finish_time,
                "eta": 0,
                "speed": 0,
                "elapsed": elapsed,
                "state": state,
    ...
    def halt_task(self, tid, elapsed, halt_time=time()):
        ...
        db_data = {
            "task_info": {"state": state, "finish_time": halt_time,},
            "task_status": {
                "pause_time": halt_time,
                "eta": 0,
                "speed": 0,
                "elapsed": elapsed,
                "state": state,
    ...
    def query_task(self, tid):
        self.db.execute(
            "SELECT * FROM task_status, task_info, task_ydl_opt WHERE task_status.tid=(?) and task_info.tid=(?) and task_ydl_opt.tid=(?)",
            (tid, tid, tid),
        )
        row = self.db.fetchone()
        if row is None:
    ...
    def progress_update(self, tid, d, elapsed):
        self.logger.debug(
            "update filename=%s, tmpfilename=%s" % (d["filename"], d["tmpfilename"])
        )
    ...
    def launch_unfinished(self):
        self.db.execute(
            "SELECT tid FROM task_status WHERE state in (?)",
